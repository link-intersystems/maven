...
public class ComponentValueSetter {
	...
	public ComponentValueSetter(final String fieldName, final Object object,
	...
	private void initSetter(final Class implementation) {
		...
		if (implementation != null
				&& setterParamType.isAssignableFrom(implementation)) {
			setterParamType = implementation; // more specific, compatible type
		}

		try {
			setterTypeConverter = lookup
					.lookupConverterForType(setterParamType);

			if (setterTypeConverter instanceof ParameterizedConfigurationConverter) {
				setterTypeArguments = getTypeArguments(setter
	...
	private static Method getAdder(final String fieldName, final Class clazz) {
		...
		final String adderName = "add"
				+ StringUtils.capitalizeFirstLetter(fieldName);
	...
	private void setValueUsingField(final Object value)
		...
		try {
			...
			if (listener != null) {
				listener.notifyFieldChangeUsingReflection(fieldName, value,
						object);
			}
			...
			throw new ComponentConfigurationException("Cannot access field: "
					+ field, e);
		} catch (final IllegalArgumentException e) {
			throw new ComponentConfigurationException("Cannot assign value '"
					+ value + "' (type: " + value.getClass() + ") to " + field,
					e);
		}
	}
	...
	private void setValueUsingSetter(final Object value)
		...
		final String exceptionInfo = object.getClass().getName() + "."
				+ setter.getName() + "( " + setterParamType.getName() + " )";
		...
		try {
			...
			throw new ComponentConfigurationException("Cannot access method: "
					+ exceptionInfo, e);
		} catch (final IllegalArgumentException e) {
			throw new ComponentConfigurationException(
					"Invalid parameter supplied while setting '" + value
							+ "' to " + exceptionInfo, e);
	...
	public void configure(final PlexusConfiguration config,
			final ClassLoader classLoader, final ExpressionEvaluator evaluator)
			throws ComponentConfigurationException {
		Object value = null;
		...
		if (setterTypeConverter != null) {
			try {
				if (setterTypeArguments != null) {
					...
					value = converter
							.fromConfiguration(lookup, config, setterParamType,
									setterTypeArguments, object.getClass(),
									classLoader, evaluator, listener);
				} else {
					value = setterTypeConverter.fromConfiguration(lookup,
							config, setterParamType, object.getClass(),
							classLoader, evaluator, listener);
				}
				...
				if (fieldTypeConverter == null
						|| fieldTypeConverter.getClass().equals(
								setterTypeConverter.getClass())) {
					throw e;
				}
			}
		}
		...
		if (fieldTypeArguments != null) {
			...
			value = converter.fromConfiguration(lookup, config, fieldType,
					fieldTypeArguments, object.getClass(), classLoader,
					evaluator, listener);
		} else {
			value = fieldTypeConverter.fromConfiguration(lookup, config,
					fieldType, object.getClass(), classLoader, evaluator,
					listener);
		}
